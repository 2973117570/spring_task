...
//		params.add(C);
		...
		System.out.println("===============更新 testUpdateData=======>>>>>>>>>>>>>:"+ret.getContent().toString());
	}
	
	public static int testCountData() throws UnsupportedEncodingException{
		Map<String, Object> paramsMap = new IdentityHashMap<String, Object>();
		int totalNum=0;
		System.out.println("start count data");
		String url =  "http://10.235.156.179:8072/execute/countQuery";
		String storeName ="dbmycat1";
		String sql ="SELECT COUNT(1) from JZ_ASJ_B_ASJ_JQ t1 where t1.xyflag is NULL and t1.FADD is not NULL ";
		...
		totalNum=(Integer) JSON.parseObject(ret.getContent().toString()).get("count");
		return totalNum;
	}
	...
	public static List<Map> testSelectData(int index,int pageSize) throws UnsupportedEncodingException{
		...
		String storeName ="dbmycat1";
		String sql ="select t1.ajbh,t1.fadd,t1.px,t1.py,t1.xyflag from JZ_ASJ_B_ASJ_JQ t1 where t1.xyflag is NULL and t1.FADD is not NULL order by t1.ajbh asc limit "+start+","+end;
